...
function internalCommandParts(command: string): string[] {
  ...
  for (const char of command.trim()) {
    ...
    if (char === ";" || char === "|" || char === "&") {
      throw new Error(
        "devspace-log is an internal bounded log query and cannot be chained with shell operators",
      );
    }
    if (/\s/.test(char)) push();
    else current += char;
  ...
  if (escaped || quote) {
    throw new Error("devspace-log contains an unterminated quote or escape");
  }
  push();
  return parts;
}

async function findRunDir(root: string, runId: string): Promise<string> {
  if (!RUN_RE.test(runId)) throw new Error("invalid run_id");
  const directDay = dayFromRunId(runId);
  if (directDay) {
    const candidate = path.join(root, directDay, runId);
    try {
      const entries = await readdir(candidate);
      if (entries.includes("output.log") || entries.includes("meta.json")) {
        return candidate;
      }
    } catch {
...
async function streamTail(filePath: string, count: number): Promise<string> {
  ...
  try {
    for await (const line of reader) {
      ring.push(line.length > MAX_RETURN_CHARS
        ? `${line.slice(0, MAX_RETURN_CHARS)}…`
        : line);
...
async function readBytes(
  ...
  const length = Number.isFinite(lengthParsed)
    ? Math.min(Math.max(lengthParsed, 1), MAX_BYTE_READ)
    : MAX_BYTE_READ;
...
export async function handleRunLogCommand(
  command: string,
  root: string,
): Promise<string | null> {
  ...
  if (!action || !runId) {
    throw new Error(
      "usage: devspace-log read|tail|grep|bytes|meta|meta-full <run_id> ...",
    );
  }
  const runDir = await findRunDir(root, runId);
  ...
  if (action === "meta" || action === "meta-full") {
    ...
    try {
      ...
      return JSON.stringify(
        compactMeta(JSON.parse(raw) as Record<string, unknown>),
        null,
        2,
      );
    } catch {
  ...
  if (action === "tail") {
    return streamTail(outputPath, boundedCount(parts[3], 80));
  }
  if (action === "read") {
  ...
  if (action === "grep") {
    ...
    if (!pattern) {
      throw new Error("usage: devspace-log grep <run_id> <pattern>");
    }
    return streamGrep(outputPath, pattern);
  }
  if (action === "bytes") {
    return readBytes(outputPath, parts[3], parts[4]);
  }

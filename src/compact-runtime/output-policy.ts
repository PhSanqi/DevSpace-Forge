...
export function commandClass(command: string): CommandClass {
  ...
  if (/\b(pytest|ctest|colcon\s+build|cmake\s+--build|make|ninja|npm\s+(test|run\s+(test|build|lint))|pnpm\s+(test|run\s+(test|build|lint))|cargo\s+(test|build|clippy)|go\s+test)\b/.test(normalized)) {
    return "verification";
  }
  return "default";
}
...
export function compactPreview(
  ...
  if (isError) {
    maxChars = Math.max(maxChars, 2_200);

...
(() => {
  ...
  function renderRuntime(d) {
    ...
    const actualRows=[row(t('pid'),actual.pid),row(t('actualRuntimeId'),rt.actual_runtime_id),row(t('runtimeTopology'),rt.runtime_topology),row(t('packageRoot'),actual.package_root,true),row(t('version'),actual.version),row(t('serverHash'),actual.server_sha256,true),row(t('source'),actual.evidence)];
    const declaredRows=[row(t('activeSlot'),rt.active_slot),row(t('configuredRuntimeId'),rt.configured_runtime_id),row(t('packageRoot'),rt.package_root,true),row(t('version'),rt.version),row(t('serverHash'),rt.server_sha256,true)];
  ...
  function renderDeployment(d){
    ...
    const recovery='<div class="panel-body">'+row(t('runtimeTopology'),dep.runtime_topology)+row(t('actualRuntimeId'),dep.actual_runtime_id)+row(t('activeSlot'),dep.active_slot)+row(t('configuredRuntimeId'),dep.configured_runtime_id)+row(t('previousSlot'),dep.previous_slot)+selector+preview+'<button class="danger-button rollback-button" data-action="rollback-runtime" '+(!chosen||!actions.rollback_runtime?'disabled':'')+'>'+h(t('runRollback'))+'</button>'+note(t('rollbackConfirmText'),'warning')+'</div>';

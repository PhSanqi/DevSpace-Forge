...
if (-not (Test-Path $csc)) { throw '.NET Framework 4.x C# compiler was not found.' }

$output = if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    Join-Path $PSScriptRoot 'bin\Setup.exe'
} else {
...
& $csc /nologo /target:winexe /optimize+ `
    ...
    /out:$output `
    (Join-Path $PSScriptRoot 'src\SetupProgram.cs') `
    (Join-Path $PSScriptRoot 'src\SetupForm.cs') `
